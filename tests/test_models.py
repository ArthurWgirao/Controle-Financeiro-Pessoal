...
def test_campos_tipos_nulabilidade_e_chaves(caminho_banco):
    ...
    assert set(metas) == {"id", "usuario_id", "categoria", "limite"}
...
def test_transacao_persiste_decimal_e_data(caminho_banco, usuario):
    transacao = Transacao(
        tipo="receita", usuario_id=usuario.id,
        valor=Decimal("10.25"),
        categoria="Outro",
        descricao="Teste",
        data=date(2026, 7, 28)
...
def test_meta_tem_categoria_unica_por_usuario(caminho_banco):
    restricoes = inspect(db.engine).get_unique_constraints("metas")
    assert any(
        restricao["column_names"] == ["usuario_id", "categoria"]
...
def test_rollback_recupera_sessao_apos_categoria_duplicada(caminho_banco, usuario):
    db.session.add(Meta(usuario_id=usuario.id, categoria="Comida", limite=Decimal("100.00")))
    db.session.commit()
    db.session.add(Meta(usuario_id=usuario.id, categoria="Comida", limite=Decimal("200.00")))
